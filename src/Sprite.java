...
public class Sprite {
    ...
    public Sprite(int x, int y, int dir, boolean needRotate) {
    ...
    public void draw(Graphics2D g2) {
        double rotationRequired = (needRotate) ? Math.toRadians(picOrientation - dir) : 0;
...
//        g2.draw(getBoundingRectangle());
    ...
    public void undoUpdate() {
        int dx = (int) (Math.cos(Math.toRadians((dir + 180) % 360)) * speed);
        int dy = -(int) (Math.sin(Math.toRadians((dir + 180) % 360)) * speed);
        loc.translate(dx, dy);
    }
